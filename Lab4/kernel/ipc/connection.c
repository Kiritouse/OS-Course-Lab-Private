                           ...
                           cap_t register_thread_cap, unsigned long destructor)
{
         ...
         * client registration.
         */
        //获取注册回调线程对象，从cap_group中通过cap获取线程对象
        register_cb_thread =
...
/*
        ...
        /*
        注册回调线程的general_ipc_config字段设置为创建的struct ipc_server_register_cb_config
        其中记录了注册回调线程（在服务器线程）的入口函数和用户态栈地址等信息。
        */
         ...
         */
        /*config为服务线程的config，其中配置了注册回调线程和IPC服务线程的入口函数*/
                             ...
                             struct client_connection_result *res)
{
        ...
        //创建连接时，初始化共享内存相关信息
        //比如共享内存在客户端的虚拟地址、大小以及在客户端和服务器端的cap
        //这里记录cap是为了方便通过cap号查找对应的pmo对象，从而进行资源回收
...
cap_t sys_register_client(cap_t server_cap, unsigned long shm_config_ptr)
{
        ...
        /*获取register_cb_thread，即服务器中的回调线程的上下文保存在config中*/
...
// Finish the IPC registration in server side
// This function is invoked by the register_cb_thread in server process
// after it is scheduled by the client process.
//完成服务器端的IPC注册
//该函数由服务器进程中的register_cb_thread调用
//在它被客户端进程调度后调用。
