...
{
        struct server_entry_node *private_iter;
/*首先，我们声明了一个用于迭代的映射节点private_iter。
接着，我们的映射表位于临界区域，会有多个进程访问。因此我们需要加上自旋锁。
然后采用我们很熟悉的链表迭代循环来寻找到记录了对应的挂载点(client_badge)的映射节点。
这里使用client_badge是因为VFS虚拟文件系统和FS文件系统是通过ipc来进行协同的,因为我们需要通过client_badge来获取具体是哪个客户在访问
